...
for (i in 1:nrow(data.ma.cbef)) {
  ...
  if (data.ma.cbef$outcome.flag[i] == "DICH"| data.ma.cbef$outcome.flag[i] == "IPD") { # dichotomous outcome
    ...
    } else if (grepl("^SMD$", data.ma.cbef$outcome.measure.merged[i])) { # ERROR was previously SDM!!
      orig = metacont(n.e = total1, mean.e = mean1, sd.e = sd1,
                      n.c = total2, mean.c = mean2, sd.c = sd2, data = d,
                      sm = "SMD", method.smd = "Hedges")
...
@
<<Save data for OSF and Erik, eval=FALSE>>=
# save(data, data.ma, data.ref, data.review, file = file.path(PATH_DATA, "PubBias-Erik.RData"))
d = subset(data, subset = !is.na(data$effect.t) & 
             !duplicated(data$effect.key)
)

d$comparator = "other"
idx = grep("placebo",d$comparison.name, ignore.case = TRUE)
d$comparator[idx] = "placebo"
d$comparator = as.factor(d$comparator)

s = paste(d$char.notes, d$char.methods, d$title, sep=" ")
d$phase = "unknown"
ind = grep("phase 1|phase I", s, ignore.case = TRUE)
d$phase[ind]="phase 1"
ind=grep("phase 2|phase II", s, ignore.case = TRUE)
d$phase[ind]="phase 2"
ind=grep("phase 3|phase III", s, ignore.case = TRUE)
d$phase[ind]="phase 3"
ind=grep("phase 4|phase IV", s, ignore.case = TRUE)
d$phase[ind]="phase 4"
d$phase = as.factor(d$phase)
library(openssl)
d$study.id.sha1 = sha1(d$study.id)

tab = d[, c("study.id.sha1", "comparison.nr", "outcome.nr", "outcome.group", "RCT", "phase", "comparator", "effect.t", "study.data_source")]
colnames(tab)[colnames(tab) == "effect.t"] = "z"
# write.csv(tab, file = "~/Data/PubBias/results/CochraneEffects.csv")

tab$is.duplicated = duplicated(tab$study.id.sha1)
erik = subset(tab, subset =  outcome.nr == 1 & !is.duplicated & RCT == "yes")
summary(erik$study.data_source)/nrow(erik)

# Erik used then the following code to sample studies:
# d=read.csv("M:\\papers\\Cochrane data\\CochraneEffects.csv")
# d=d %>% filter(RCT=="yes")
...
\setcounter{figure}{1} % start first table with Figure 2
...
plot.1.OR = ggplot(tab, aes(y=variables, x=theta, color=mycolor)) +
  ...
  scale_colour_manual(values=c(COLORS4[c(2,1)], "grey50"), labels = c("higher\nodds", "lower\nodds", "n.s.")) +
  ...
  ggtitle(sprintf("Statistical significance of\n%s study effects",  prettyNum(nrow(d.1.efficacy), big.mark = ",")))
@
...
tab = rbind(tab1,tab2)
rownames(tab)[1] = paste("Specialty:",rownames(tab)[1])
rownames(tab)[(n+1):nrow(tab)] = c("Median SJR (log2)", "Sample size (log2)", "Median citations per year (log2)", 
...
rownames(tab2) = c("Median SJR (log2)", "Sample size (log2)", "Median citations per year (log2)", 
...
rownames(tab)[1] = paste("Specialty:",rownames(tab)[1])
print(xtable(tab, align = rep("r", ncol(tab)+1), 
             caption = "Regression analysis of variables associated with the t-score from adjustment by regression. In the medical specialties, Emergency and Trauma is closest to the mean estimate across all specialties and severs as baseline level.", label = "label"),
...
plot.3.stat = ggplot(tab, aes(y=variables, x=Estimate, color=mycolor)) +
  ...
  scale_colour_manual(values=c(COLORS4[c(1,2)], "grey50"), labels = c("less\nbias", "more\nbias", "n.s.")) +
...
plot.3.OR = ggplot(tab, aes(y=variables, x=theta, color=mycolor)) +
  ...
  theme_minimal() + theme(legend.position = "none",#c(0.18, 0.92), 
                          legend.title = element_blank(),
                          legend.key.height = unit(0.3, "cm"),
                          plot.title = element_text(size=PLOT_FSIZE),
                          axis.text = element_text(size = PLOT_FSIZE),
...
plot.3.reviews = ggplot(tab, aes(x = specialty, y=nr, fill=group)) +
  ...
  theme(legend.title = element_blank(), 
        ...
        axis.title = element_text(size = PLOT_FSIZE),
        legend.text = element_text(size = PLOT_FSIZE-2),
...
#tab2 = cbind(s$coefficients[(n+1):nrow(s$coefficients),], ci[(n+3):nrow(ci),])
tab2 = s$coefficients[(n+1):nrow(s$coefficients),]
rownames(tab2) = c("Median SJR (log2)", "Sample size (log2)", "Med. citations per year (log2)", 
                   "Review year (per 10 years)", "No. of studies (log2)")
...
rownames(tab)[1] = paste("Specialty:",rownames(tab)[1])
print(xtable(tab, align = rep("r", ncol(tab)+1), 
             caption = "Regression analysis of variables associated with the change of effect estimates after adjustment by regression. In the medical specialties, Hepato-Billary is closest to the mean estimate across all specialties and severs as baseline level.", label = "label"),
      hline.after = c(0, n-1, nrow(tab)), size = "scriptsize", table.placement = "H")
#tableRegression(fit.4, xtable = F)
@
...
plot.4.reg = ggplot(tab, aes(y=variables, x=Estimate, color=mycolor)) +
  ...
  theme_minimal() + theme(legend.position = c(0.83, 0.10), 
                          ...
                          axis.title.y = element_blank()) +
  geom_vline(xintercept = 0, linetype="dashed", color = "grey50", size=0.5) + 
  xlab("estimate for change in effect r") +
  ggtitle("Change in effect (regression)")
@
...
rownames(tab)[1] = paste("Specialty:",rownames(tab)[1])
print(xtable(tab, align = rep("r", ncol(tab)+1), 
             caption = "Regression analysis of variables associated with change of the effect estimates after adjustment by Copas selction model. In the medical specialties, Lungs is closest to the mean estimate across all specialties and severs as baseline level.", label = "label"),
      hline.after = c(0, n-1, nrow(tab)), size = "scriptsize", table.placement = "H")
#tableRegression(fit.4, xtable = F)
@
...
plot.4.regCopas = ggplot(tab, aes(y=variables, x=Estimate, color=mycolor)) +
  ...
  geom_vline(xintercept = 0, linetype="dashed", color = "grey50", size=0.5) + 
  xlab("estimate for change in effect r") +
  ggtitle("Change in effect (Copas)")
@
...
fig.4  = ggdraw() +
  draw_plot(plot.4.adjhist, x = 0, y = .68, width = .50, height = .32) +
  draw_plot(plot.4.cophist, x = .50, y = .68, width = .50, height = .32) +
  draw_plot(plot.4.reg, x = 0, y = .32, width = .50, height = .36) +
  draw_plot(plot.4.regCopas, x = .50, y = .32, width = .50, height = .36) +
...
tiff(file.path(PATH_FIGURES, 'Figure4.tiff'), width = 7, height = 8.5, units = "in", res = 300)
...
for (i in 1:length(idx)) {
  ...
  mtext(substr(d.ma$comparison.name,1,38), side=3, line=1.0, cex = 0.5)
  mtext(substr(d.ma$outcome.name,1,38), side=3, line=0.4, cex = 0.5)
...
d = subset(data.ma, subset = idx)
nrow(d)

sum(d$I2 < 50)
d$hetero = NA
d$hetero[d$I2 < 50] = "low"
d$hetero[d$I2 >= 50] = "high"
d$hetero = as.factor(d$hetero)
summary(d$hetero)
d$mean_sample_size = d$N/d$nr.studies
tapply(d$N, d$hetero, median)
tapply(d$N/d$nr.studies, d$hetero, median)

ggplot(d, aes(y=log(mean_sample_size), x=hetero, color=hetero)) +
  geom_boxplot() + 
  geom_point(size=0.5, shape=1, position = position_jitter(width = 0.2, height = 0), alpha=0.1) +
  theme(legend.position = "none") + theme_minimal() +
  scale_color_manual(values = COLORS4) + xlab("heterogeneity") + ylab("log(average study size per meta-analysis)")
@
<<Heterogeneity 2, eval=FALSE>>=
idx = data.ma$estimable == "YES" &
  data.ma$outcome.group == "efficacy" & 
  # data.ma$outcome.flag != "IV" & # IV outcomes have no data do reanalyze MA
  data.ma$I2 > 80 &
  ...
  data.ma$total1 > 0 & 
  data.ma$total2 > 0 &# few IV have zero group sizes
  data.ma$outcome.measure.merged == "RR" &
  data.ma$outsub.id == "CMP-001.01"

i = 510
d.ma = data.ma[i,]
d = data[data$id == "CD000023" & data$comparison.nr == 1 & data$outcome.nr == 1,]

ma.rr = metabin(event.e = events1, n.e = total1, 
                event.c = events2, n.c = total2, RR.Cochrane = TRUE, 
                Q.Cochrane = TRUE, sm = "RR", data = d)
ma.or = metabin(event.e = events1, n.e = total1, 
                event.c = events2, n.c = total2, RR.Cochrane = TRUE, 
                Q.Cochrane = TRUE, sm = "OR", data = d)

# so tau is substatially different while I^2 is not

@

<<Missing data, eval=FALSE>>=
summary(d.1.efficacy$effect.r)
summary(d.2.efficacy$ma.r)
summary(data.ma.incl$pb.stat)

sum(is.na(data.ma.incl$reg.delta))/nrow(data.ma.incl)
sum(is.na(data.ma.incl$cop.delta))/nrow(data.ma.incl)

d = d[, c("p", "specialty", "SJR.Best.Quartile", "effect.N", "cited.peryear", "year", "RCT")]
summary(d)
...
sum(is.na(d$SJR.Best.Quartile) | is.na(d$cited.peryear) | is.na(d$year))/ nrow(d)
...
sum(is.na(d$year))/nrow(d)
@
<<Category boundaries for SJR, eval=FALSE>>=
range(data$SJR[data$SJR.Best.Quartile == "Q1"], na.rm = TRUE)
range(data$SJR[data$SJR.Best.Quartile == "Q2"], na.rm = TRUE)
range(data$SJR[data$SJR.Best.Quartile == "Q3"], na.rm = TRUE)
range(data$SJR[data$SJR.Best.Quartile == "Q4"], na.rm = TRUE)
# proper categorization was done by SCIMago and is subfield specific
@
...
d$rx
@
<<Studies with both dichotomous and binary outcome from Psychiatry>>=
d = subset(d.1.efficacy, subset = specialty == "Psychiatry & Mental Health" & comparison.nr == 1)
myStudies = unique(d$study.id)
N = length(myStudies) # no of studies

i = 1
tmp = subset(d, subset = study.id == myStudies[i])



@

<<Checking out meta-analysis recalculation, eval=FALSE>>=

# Original analysis from Cochrane
data.ma.cbef[3642, c("effect", "ci_start", "ci_end", "z", "p_z")]
# Simon's reanalysis using the original effect measure
data.ma.cbef[3642, c("orig.effect.rx", "orig.se.rx", "orig.lower.rx", "orig.upper.rx", "orig.z.rx", "orig.p.rx", "orig.measure")]
# Simon's meta-analysis on effect measures using SMD or OR
data.ma.cbef[3642, c("ma.effect.rx", "ma.se.rx", "ma.lower.rx", "ma.upper.rx", "ma.z.rx", "ma.p.rx", "ma.measure")]


# Cochrane
data.ma.cbef[950, c("effect", "ci_start", "ci_end", "z", "p_z", "outcome.measure.merged")]
# Simon's reanalysis on original effect measure
data.ma.cbef[950, c("orig.effect.fx", "orig.se.fx", "orig.lower.fx", "orig.upper.fx", "orig.z.fx", "orig.p.fx", "orig.measure")]
# Simon's meta-analyis on heterogenized effect measures SMD a OR
data.ma.cbef[950, c("ma.effect.fx", "ma.se.fx", "ma.lower.fx", "ma.upper.fx", "ma.z.fx", "ma.p.fx", "ma.measure")]

idx = data.ma.cbef$z > -20 & data.ma.cbef$z < 20
hist(data.ma.cbef$z[idx])

idx = data.ma.cbef$ma.z.fx > -20 & data.ma.cbef$ma.z.fx < 20
hist(data.ma.cbef$ma.z.fx[idx])

plot(data.ma.cbef$z[idx], abs(data.ma.cbef$ma.z.rx[idx]), xlim = c(0,20), ylim = c(0,20), cex=0.1)
@
