...
data$effect.es[idx] = tmp$g
data$effect.se[idx] = tmp$SEg
...
patternAdverse = "toxicity|adverse|side effect"
...
sprintf("%d not estimtable; %d nr studies < 2; %d not efficacy; %d has duplicated effects; %d total",
        sum(d$estimable == "NO"),
...
summary(d$outcome.group)/nrow(d)
...
fit.1.rx = glmer((p < 0.05) ~ specialty + SJR.Best.Quartile + log2(`effect.N`) + 
                   log2(cited.peryear) + year.10  + RCT + (1 | study.id) , family = binomial (link ="logit"), nAGQ = 0, data = d)
...
fit.2.rx = glmer((orig.p.rx < 0.05) ~ specialty + log2(`median SJR`) + log2(`sample size`) + log2(`median citations per year`) + 
                 `review year` + log2(`number of studies`) + (1 | id) + (1 | IDCMP), family = binomial (link ="logit"), nAGQ = 0, data=d)
...
plot.3.prev = ggplot(tab.propPB, aes(x=specialty, y=p)) +
  ...
  ylab("proportion") +
  ggtitle(sprintf("Prevalence of funnel plot asymmetry in\n%s meta-analyses",  prettyNum(nrow(data.ma.incl), big.mark = ",")))
...
fit.3a.rx = lmer(abs(pb.stat) ~ specialty + log2(pb.SJR) + log2(`sample size`) + log2(pb.cited.peryear) +
                  `pb.review.year` + log2(`number of studies`) + (1 | IDCMP), data=d)
...
plot.3.stat = ggplot(tab, aes(y=variables, x=Estimate, color=mycolor)) +
  ...
  xlab("estimate for evidence of asymmetry") +
  ggtitle(sprintf("Funnel plot asymmetry (t-score) \nin %s meta-analyses",  prettyNum(nrow(data.ma.incl), big.mark = ",")))
@
...
fit.3b.rx = glmer(pb.hasPubBias ~ specialty +  log2(pb.SJR) + log2(`sample size`) + 
                   log2(pb.cited.peryear) + `pb.review.year` + log2(`number of studies`) + (1 | IDCMP), family = binomial(link ="logit"), nAGQ = 1L, data=d,                                  # control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5))
)
...
plot.3.OR = ggplot(tab, aes(y=variables, x=theta, color=mycolor)) +
  ...
  theme_minimal() + theme(legend.position = c(0.18, 0.92), 
                          ...
                          axis.title.y = element_blank()) +
  geom_vline(xintercept = 0, linetype="dashed", color = "grey50", size=0.5) + xlab("odds ratio") +
  ggtitle(sprintf("Funnel plot asymmetry (yes/no) \nin %s meta-analyses",  prettyNum(nrow(data.ma.incl), big.mark = ",")))
@
...
plot.3.catslope = ggplot(d, aes(y=pb.slope, x=category, color=category)) +
  ...
  scale_x_discrete(labels = c("no asymmetry", "asymmetry", "asymmetry and\nprobable publication bias")) +
...
plot.3.reviews = ggplot(tab, aes(x = specialty, y=nr, fill=group)) +
  ...
  geom_text(aes(label=ifelse(group=="ppb", as.character(specialty), "")), vjust=0.1, hjust=0,
            color="black", size=2.5, angle=90) + 
  scale_fill_manual(values=COLORS4[c(2,1)], labels = c("asymmetry & probable\npublication bias", "asymmetry")) +
...
for (i in 1:length(idx)) {
  ...
  if (d.ma$outcome.flag == "DICH") {
    points(exp(reg$TE[d$p < 0.05]), reg$seTE[d$p < 0.05], col=COLORS4[4], cex=0.5)
  } else if (d.ma$outcome.flag == "CONT") {
    points(reg$TE[d$p < 0.05], reg$seTE[d$p < 0.05], col=COLORS4[4], cex=0.5)
  }
...
mtext(sprintf("%.2f (from %.2f to %.2f)", exp(d.ma$cop.TE.adj), exp(d.ma$cop.lower.adj), exp(d.ma$cop.upper.adj)),
      side=1, line=5.0, adj = 0, cex = 0.7, col = COLORS4[1])
@
<<Try Egger's regression with lm, eval=FALSE>>=
fit = lm(I(effect/se) ~ I(1/se), data = d)
summary(fit)

metabias(ma2, method.bias = "linreg")
@
<<For Erik: Compare t statistic to test statistic bases on original data, eval=FALSE>>=
d = d.1.efficacy
